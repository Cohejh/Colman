...
class GUI:
    def __init__(self, master):
        ...
        self.running = False
        self.start_button = tk.Button(left_frame, text="Play", command=self.on_start_button_listener)
        ...
        label = tk.Label(separator_frame, text="Engine Parameters")
        ...
        self.slow_mover_label = tk.Label(slow_mover_frame, text="Calculation time (ms) ")
        self.slow_mover_label.pack(side=tk.LEFT)
        self.slow_mover = tk.IntVar(value=1000)
        ...
        # Engine path: Change to install.
        self.engine_path = "C:/Users/jcohe/Desktop/Colman/engine.exe"
            ...
            try:
                if self.stockfish_bot_pipe is not None and self.stockfish_bot_pipe.poll():
                    ...
                    if data == "START":
                        ...
                        tk.messagebox.showerror("Error", "Engine path provided is not valid!")
                    elif data[:8] == "ERR_PERM":
                        tk.messagebox.showerror("Error", "Engine provided is not executable!")
                    elif data[:9] == "ERR_BOARD":
                        tk.messagebox.showerror("Error", "Can't find board!")
                    elif data[:9] == "ERR_COLOR":
                        tk.messagebox.showerror("Error", "Can't find player color!")
                    elif data[:9] == "ERR_MOVES":
                        tk.messagebox.showerror("Error", "Can't find moves list!")
    ...
    def on_start_button_listener(self):
        ...
        if slow_mover < 10 or slow_mover > 10000:
            tk.messagebox.showerror("Error", "Slow Mover must be between 10 and 10000, 1000 reccomended.")
            return

        # Check if stockfish path is not empty
        if self.engine_path == "":
            tk.messagebox.showerror("Error", "Engine path is empty")
            return
